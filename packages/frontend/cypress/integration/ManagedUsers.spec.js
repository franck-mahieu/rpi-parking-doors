...
describe('ManagedUsers', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', 'http://localhost:3000/api/users/all', {
      fixture: 'usersAll',
    }).as('usersAll');
    cy.intercept('PUT', 'http://localhost:3000/api/users/add', {
      fixture: 'addUser',
    }).as('addUser');
    cy.intercept('DELETE', 'http://localhost:3000/api/users/remove', {
      fixture: 'removeUser',
    }).as('removeUser');
    cy.intercept('POST', 'http://localhost:3000/api/users/updatepassword', {
      fixture: 'updatePassword',
    }).as('updatePassword');
  });

  it('should show table and button on managed users page ', () => {
    cy.visit('http://localhost:3001/admin/manageusers');
